...
@test "alias: dot changes directory to DOTFILES" {
  # Use _zsh so the alias is visible; check we land in the dotfiles directory.
  # DOTFILES is set by .zshrc to $HOME/dotfiles which is this repo on the user's machine.
  run _zsh 'dot && pwd'
  [ "$status" -eq 0 ]
  [[ "$output" == *"dotfiles"* ]]
}
